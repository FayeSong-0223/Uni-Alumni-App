...
| Password reset emails don't arrive | By default (no SMTP credentials set), emails are **printed to the backend console** instead of being delivered — look for the reset code in the server log. To send real email, set `EMAIL_HOST_USER` and `EMAIL_HOST_PASSWORD` in `backend/.env` to a Gmail address + a [Google App Password](https://support.google.com/accounts/answer/185833) (not your normal Gmail password). |
...
| **SMTP credentials** | `backend/.env` `EMAIL_HOST_USER` / `EMAIL_HOST_PASSWORD` | No credentials are embedded in source. Set both env vars to send real mail; leave them unset to use the console backend (mail prints to stdout). Any credentials previously committed to git history should be considered compromised and rotated. |
